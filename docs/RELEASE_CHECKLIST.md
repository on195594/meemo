...
- [ ] Inventory every legacy namespace and candidate backup, exclude fixture/rehearsal collections, and bind the selected authoritative source hash plus expected corpus counts to the migration evidence.
...
11. Under the same freeze, verify readiness, authenticate as the real migrated account, and check every migrated identity class, representative historical Things, attachments, public sharing, export, restart persistence, and final migration counts.
