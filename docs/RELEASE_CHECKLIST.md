...
8. With only the explicitly controlled migration process able to write, run final user-migration `apply` and `verify`, then v2 data-migration `apply` and `verify` with the same independently reviewed expected-source artifact; compare canonical users and per-owner things/tags/settings counts. Run tag reconstruction during this freeze; require its durable MongoDB gate to report zero active Thing writers before it stages or swaps tags, and keep the gate frozen through readiness.
