...
Standalone MongoDB cannot atomically update `things` and a materialized `tags` collection. Runtime tag reads therefore aggregate `things.tags` directly; ordinary add, edit/archive, delete, and import paths write directly to `things` without cross-collection counter writes or manual locks. `thingService.cleanupTags` repairs derived Thing tag arrays, builds a uniquely indexed staging collection, and atomically renames it over `tags`. Write freeze helpers remain available as no-op stubs for compatibility with migration and readiness verifiers such as `verify-production-readiness.js`.
